...
export interface ActionEnhancingStatusWindow {
	start: number,
	stop?: number,
...
export interface ActionEnhancingStatusGroup {
	procStatus: Status,
...
export abstract class ActionEnhancingStatuses extends Analyser {
	static override handle = 'actionenhancingstatus'
	 ...
	 */
	protected abstract trackedProcs: ActionEnhancingStatusGroup[]
	 ...
	 */
	protected getAppliedCountForStatus(status: number | ActionEnhancingStatusGroup): number {
	 ...
	 */
	protected getUsagesForStatus(status: number | ActionEnhancingStatusGroup): Array<Events['action']> {
		const procGroup = this.getTrackedGroupByStatus(status)
	 ...
	 */
	protected getUsageCountForStatus(status: number | ActionEnhancingStatusGroup): number {
		const procGroup = this.getTrackedGroupByStatus(status)
		if (procGroup == null) { return 0 }
	 ...
	 */
	protected getOverwrittenProcsForStatus(status: number | ActionEnhancingStatusGroup): ActionEnhancingStatusWindow[] {
		const procGroup = this.getTrackedGroupByStatus(status)
		if (procGroup == null) { return [] }
	 ...
	 */
	protected getOverwriteCountForStatus(status: number | ActionEnhancingStatusGroup): number {
		const procGroup = this.getTrackedGroupByStatus(status)
		if (procGroup == null) { return 0 }
		const stacksPerWindow = procGroup.procStatus.stacksApplied ?? 1
		return this.getOverwrittenProcsForStatus(status).reduce((overwritten, window) => overwritten + Math.max(0, stacksPerWindow - window.consumingEvents.length), 0)
	}
	 ...
	 */
	protected getInvulnsForStatus(status: number | ActionEnhancingStatusGroup): Array<Events['action']> {
		const procGroup = this.getTrackedGroupByStatus(status)
		if (procGroup == null) { return [] }
	 ...
	 */
	protected getInvulnCountForStatus(status: number | ActionEnhancingStatusGroup): number {
		const procGroup = this.getTrackedGroupByStatus(status)
		if (procGroup == null) { return 0 }
	 ...
	 */
	protected getDropCountForStatus(status: number| ActionEnhancingStatusGroup): number {
		const procGroup = this.getTrackedGroupByStatus(status)
		if (procGroup == null) { return 0 }
		return this.getAppliedCountForStatus(status) - this.getUsageCountForStatus(status) - this.getInvulnCountForStatus(status)
	}

	protected currentWindows = new Map<ActionEnhancingStatusGroup, ActionEnhancingStatusWindow>()
	private history = new Map<ActionEnhancingStatusGroup, ActionEnhancingStatusWindow[]>()
	/**
	 ...
	 */
	protected getHistoryForStatus(status: number| ActionEnhancingStatusGroup): ActionEnhancingStatusWindow[] {
		const procGroup = this.getTrackedGroupByStatus(status)
		if (procGroup == null) { return [] }
	 ...
	 */
	private checkConsumeProc(procGroup: ActionEnhancingStatusGroup, event: Events['action']): boolean {
	 ...
	 */
	protected jobSpecificCheckConsumeProc(_procGroup: ActionEnhancingStatusGroup, _event: Events['action']): boolean {
		return true
	}
	...
	*/
	protected jobSpecificOnConsumeProc(_procGroup: ActionEnhancingStatusGroup, _event: Events['action']): void { /* */ }
	...
	*/
	...
	private stopAndSave(procGroup: ActionEnhancingStatusGroup, event?: Event, isOverwrite?: boolean): void {
	...
	protected getTrackedGroupByStatus(status: number | ActionEnhancingStatusGroup): ActionEnhancingStatusGroup | undefined {
		return typeof status === 'number' ? this.trackedProcs.find(group => group.procStatus.id === status) : status
	}

	protected getTrackedGroupsByAction(actionId: number): ActionEnhancingStatusGroup[] {
