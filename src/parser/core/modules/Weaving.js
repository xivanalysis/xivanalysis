	...
	_onComplete() {
		...
		if (badWeaves.length) {
			this.suggestions.add(new Suggestion({
						...
						content: <Fragment>
						Avoid weaving more actions than you have time for in a single GCD window. Doing so will delay your next GCD,
						reducing possible uptime. Check the <em>{this.name
					} <
 				/
				em >
				module;
			...
			Fragment > ,
				severity:;
			badWeaves.length > MAJOR_SUGGESTION_ISSUES ? SEVERITY.MAJOR : SEVERITY.MEDIUM,
				why:; `${
			badWeaves.length} instances of incorrect weaving.`;,
		}))
}
}
...
output();
{
	...
	const panels = badWeaves.map(item => ({
		...
		title: {
			content: <Fragment>
			<strong>{this.parser.formatTimestamp(item.gcdEvent.timestamp)
}
<;/
strong >  & nbsp;
- & nbsp;
