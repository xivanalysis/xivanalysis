...
export default class Module {
	...
	static get handle() {
		if (!this._handle) {
			...
			console.error(
				`\`${this.name}\` does not have an explicitly set handle. Using \`${handle
				}\`. This WILL break in minified builds.`);
			this._handle = handle;
		}
		return this._handle;
	}
	...
	set name(value) {
		console.warn(
			...
			}\` is setting the display title via the \`name\` instance property. Use \`static title\` instead.`);
	...
	constructor(parser) {
		...
		props.forEach(name => {
			...
			console.warn(
				`The \`${this.constructor.handle}\` module is using the old-style event hook \`${name
				}\`. Please update it to use the new \`addHook\` function.`);
