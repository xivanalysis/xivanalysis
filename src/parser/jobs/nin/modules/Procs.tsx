...
export class Procs extends CoreProcs {
	...
	protected override addJobSpecificSuggestions(): void {
		...
		this.suggestions.add(new TieredSuggestion({
			icon: this.data.statuses.PHANTOM_KAMAITACHI_READY.icon,
			content: <Trans id="nin.procs.suggestions.dropped-pk-ready.content">
				Avoid letting <StatusLink status="PHANTOM_KAMAITACHI_READY"/> expire without using <ActionLink action="PHANTOM_KAMAITACHI"/>, as it has considerable potency and generates 10 Ninki.
			</Trans>,
			tiers: DROP_SEVERITY,
			value: droppedPhantomKamaitachis,
			why: <Trans id="nin.procs.suggestions.dropped-pk-ready.why">
				You allowed a Phantom Kamaitachi Ready proc to expire <Plural value={droppedPhantomKamaitachis} one="# time" other="# times" />.
			</Trans>,
		}))

		const droppedTenriJindos: number = this.getDropCountForStatus(this.data.statuses.TENRI_JINDO_READY.id)
		this.suggestions.add(new TieredSuggestion({
			icon: this.data.statuses.TENRI_JINDO.icon,
			content: <Trans id="nin.procs.suggestions.dropped-tj-ready.content">
				Avoid letting <StatusLink status="TENRI_JINDO_READY"/> expire without using <ActionLink action="TENRI_JINDO"/>, as it has very high potency and should always fall under 2-minute buff windows.
			</Trans>,
			tiers: DROP_SEVERITY,
			value: droppedTenriJindos,
			why: <Trans id="nin.procs.suggestions.dropped-tj-ready.why">
				You allowed a Tenri Jindo Ready proc to expire <Plural value={droppedTenriJindos} one="# time" other="# times" />.
			</Trans>,
		}))
	}
}
