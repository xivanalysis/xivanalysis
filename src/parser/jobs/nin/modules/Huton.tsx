...
// TODO - These numbers gettin' CHAAAAAAANGED (in EW)
...
export default class Huton extends Analyser {
	...
	private highEstimate: HutonEstimate = {
		current: HUTON_START_DURATION_MILLIS.high - (BOSS_ADJUSTMENTS[this.parser.fight.boss] || 0),
		clipped: 0,
		downtime: 0,
		badAcs: 0,
	}
	private lowEstimate: HutonEstimate = {
		current: HUTON_START_DURATION_MILLIS.low - (BOSS_ADJUSTMENTS[this.parser.fight.boss] || 0),
		clipped: 0,
		downtime: 0,
		badAcs: 0,
	}
	...
	override initialise() {
		...
		this.addEventHook(playerFilter.type('action').action(ACTIONS.HUTON.id), this.onHutonCast)
		this.addEventHook(playerFilter.type('combo').action(oneOf(Object.keys(HUTON_EXTENSION_MILLIS).map(Number))), this.onHutonExtension)
		this.addEventHook(playerFilter.type('death'), this.onDeath)
		this.addEventHook(playerFilter.type('raise'), this.onRaise)
	...
	private onHutonExtension(event: Events['action']) {
		const elapsedTime = (event.timestamp - this._lastEventTime)
