	...
	_onComplete() {
		if (!this._shadewalkerUsed) {
			this.suggestions.add(new Suggestion({
				...
				content: <Fragment>
					<Trans id="nin.utility.suggestions.no-shade.content"><ActionLink {...ACTIONS.SHADEWALKER}/> should almost always be used on the pull, as it will help your main tank solidify threat much more quickly and reduce everyone else&apos;s risk of stealing threat.</Trans>
				</Fragment>,
				severity: SEVERITY.MEDIUM,
				why: <Fragment>
					<Trans id="nin.utility.suggestions.no-shade.why">You didn&apos;t use Shadewalker pre-pull or within the first {this.parser.formatDuration(SHADEWALKER_EXPECTED_BY_MILLIS)} of the fight.</Trans>
				</Fragment>,
			}))
		}
		...
		if (this._shadewalkerDuration >= 0 && this._shadewalkerDuration < SHADEWALKER_PREPULL_MILLIS) {
			this.suggestions.add(new Suggestion({
				icon: ACTIONS.SHADEWALKER.icon,
				content: <Fragment>
					<Trans id="nin.utility.suggestions.early-shade.content">Avoid using <ActionLink {...ACTIONS.SHADEWALKER}/> too early on the countdown. If using it pre-pull, you should ideally start the fight with at least {this.parser.formatDuration(SHADEWALKER_PREPULL_MILLIS)} of the buff remaining.</Trans>
				</Fragment>,
				severity: SEVERITY.MINOR,
				why: <Fragment>
					<Trans id="nin.utility.suggestions.early-shade.why">Shadewalker only had {this.parser.formatDuration(this._shadewalkerDuration)} left when the fight started, reducing its effectiveness.</Trans>
				</Fragment>,
			}))
		}
		...
		if (!this._smokeScreenUsed) {
			this.suggestions.add(new Suggestion({
				...
				content: <Fragment>
					<Trans id="nin.utility.suggestions.no-smoke.content"><ActionLink {...ACTIONS.SMOKE_SCREEN}/> can be a very useful utility for your party and should be used to help mitigate the threat generated by high burst DPS or burst healing windows.</Trans>
				</Fragment>,
				severity: SEVERITY.MINOR,
				why: <Fragment>
					<Trans id="nin.utility.suggestions.no-smoke.why">You didn&apos;t use Smoke Screen pre-pull or within the first {this.parser.formatDuration(SMOKE_SCREEN_EXPECTED_BY_MILLIS)} of the fight.</Trans>
				</Fragment>,
			}))
		}
		...
		if (this._smokeScreenDuration >= 0 && this._smokeScreenDuration < SMOKE_SCREEN_PREPULL_MILLIS) {
			this.suggestions.add(new Suggestion({
				icon: ACTIONS.SMOKE_SCREEN.icon,
				content: <Fragment>
					<Trans id="nin.utility.suggestions.early-smoke.content">Avoid using <ActionLink {...ACTIONS.SMOKE_SCREEN}/> too early on the countdown. If using it pre-pull, you should ideally start the fight with at least {this.parser.formatDuration(SMOKE_SCREEN_PREPULL_MILLIS)} of the buff remaining.</Trans>
				</Fragment>,
				severity: SEVERITY.MINOR,
				why: <Fragment>
					<Trans id="nin.utility.suggestions.early-smoke.why">Smoke Screen only had {this.parser.formatDuration(this._smokeScreenDuration)} left when the fight started, reducing its effectiveness.</Trans>
				</Fragment>,
			}))
		}
	}
