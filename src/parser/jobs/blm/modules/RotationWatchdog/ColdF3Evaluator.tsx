...
export class ColdF3Evaluator extends RulePassedEvaluator {
	...
	override passesRule(window: HistoryEntry<EvaluatedAction[]>) {
		...
		return !window.data.some((event) => event.action.id === this.fire3Action.id && this.gauge.getGaugeState(event.timestamp - 1).umbralIce === ASTRAL_UMBRAL_MAX_STACKS && this.gauge.getGaugeState(event.timestamp - 1).umbralHearts === UMBRAL_HEARTS_MAX_STACKS)
	}
	...
	override suggest(windows: Array<HistoryEntry<EvaluatedAction[]>>) {
		...
		return new TieredSuggestion({
			...
			why: <Trans id="blm.rotation-watchdog.suggestions.coldf3.why">
				<Plural value={rotationsWithColdF3} one="# Astral Fire phase" other="# Astral Fire phases"/> began with a weakend <DataLink showIcon={false} action="FIRE_III" />.
			</Trans>,
		})
	}
}
