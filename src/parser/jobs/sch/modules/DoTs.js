	...
	addChecklistRules() {
		this.checklist.add(new TieredRule({
			...
			description: <Trans id="sch.dots.checklist.description">
				As a Scholar, Biolysis is a notable porition of your damage. Aim to keep it up at all times.
			</Trans>,
	...
	addClippingSuggestions(clip) {
		...
		this.suggestions.add(new TieredSuggestion({
			...
			why: <Trans id="sch.dots.suggestions.clipping.why">
				An average of {this.parser.formatDuration(this.getClippingAmount(STATUSES.BIOLYSIS.id) * 1000)} of <StatusLink {...STATUSES.BIOLYSIS}/> clipped every minute, for a total of {this.parser.formatDuration(clip[STATUSES.BIOLYSIS.id])} lost to early refreshes.
			</Trans>,
		}))
	}
}
