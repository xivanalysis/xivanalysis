...
export class Weaving extends CoreWeaving {
	override getMaxWeaves(weave: Weave) {
		...
		const hasPhoenixSummon = weave.weaves.some(weave => weave.action === ACTIONS.FIREBIRD_TRANCE.id)
		...
		if (leadingGcdCastTime === 0 && weave.weaves[1].action === ACTIONS.FIREBIRD_TRANCE.id) {
