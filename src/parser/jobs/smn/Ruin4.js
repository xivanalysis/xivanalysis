	...
	_onComplete() {
		...
		if (this._overage > 0) {
			this.suggestions.add(new Suggestion({
					...
					content: <Fragment>
					Use <ActionLink {...ACTIONS.RUIN_IV
				} /
 				>
				as;
			soon;
			...
			Fragment > ,
				severity:;
			this._overage > 30000 ? SEVERITY.MAJOR : this._overage > 10000 ? SEVERITY.MEDIUM : SEVERITY.MINOR,
				why:; `Further Ruin held for ${
			this.parser.formatDuration(this._overage)} longer than recommended over the course of the fight.`;
			,
		}))
}
}
...
output();
{
	...
	br /  >
		Expected;
	procs: {
		Math.floor(this._procChances * PROC_RATE)
	}<
	br /  >
		Actual;
	procs: {
