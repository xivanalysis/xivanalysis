	...
	_onComplete(event) {
		...
		if (numCasters > 1 && mostUsedPet !== PETS.GARUDA_EGI.id) {
			this.suggestions.add(new Suggestion({
					...
					content: <Fragment>
					You should be primarily using Garuda-Egi when in parties with casters other than yourself - they will benefit from
					<
					ActionLink {...ACTIONS.CONTAGION
				} /
 				>
 				&
				apos;
...
if (numCasters === 1 && mostUsedPet !== PETS.IFRIT_EGI.id) {
	this.suggestions.add(new Suggestion({
		...
		severity: SEVERITY.MEDIUM,
		content: <Fragment>
		You should be primarily using Ifrit-Egi when there are no other casters in the party - Ifrit&apos;
	s;
	raw;
	damage;
	and < ActionLink;
...
if (titanUptimePercent > 5) {
	this.suggestions.add(new Suggestion({
		...
		content: <Fragment>
		Titan-Egi generally should not be used in party content. Switch to Ifrit-Egi,
		or Garuda-Egi if you have multiple casters.
		</Fragment>,
	}));
}
...
if (noPetUptimePercent > 1) {
	this.suggestions.add(new Suggestion({
		...
		content: <Fragment>
		Pets provide a <em>lot</em> of SMN&apos;
	s;
	passive;
	damage, and;
	are;
...
output();
{
	...
	return <;
	Fragment >  < div;
	className = { styles.chartWrapper } >  < PieChart;
	data = { data };
	options = { options };
	width = { 100 };
	height = { 100 } /  >  < /;
...
/>
<;/
td >  < td > { this.getPetName(petId) } < /;
td >  < td > { this.parser.formatDuration(this._petUptime.get(petId))}
<;/
td >  < td > { this.getPetUptimePercent(petId) } %  < /;
td >  < /;
tr > )}
;</
tbody >  < /;
