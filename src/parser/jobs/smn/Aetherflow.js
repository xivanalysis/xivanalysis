...
export default class Aetherflow extends Module {
	...
	_onComplete() {
		...
		this.checklist.add(new Rule({
					name: <Fragment>Use <ActionLink {...ACTIONS.AETHERFLOW
				} /
 				>
				effectively <
 			/
		...
		[
			new Requirement({
					name: <Fragment><ActionLink {...ACTIONS.AETHERFLOW
				} /
 				>
				cooldown;
		uptime < /;
		Fragment > ,
			percent:;
		(this.cooldowns.getTimeOnCooldown(ACTIONS.AETHERFLOW.id) / (this.parser.fightDuration - 15000)) * 100, ;
	}),
	],
}))
...
if (numBadFesters) {
	this.suggestions.add(new Suggestion({
			...
			content: <Fragment>
			To get the most potency out of your <ActionLink {...ACTIONS.FESTER
		} /
 		>
		s, ensure;
	both < StatusLink;
	...
	Fragment > ,
		severity:;
	numBadFesters < 5 ? SEVERITY.MEDIUM : SEVERITY.MAJOR,
		why:; <
	Fragment >
		{ numBadFesters };
	cast;
...
if (numBadPainflares) {
	this.suggestions.add(new Suggestion({
			...
			content: <Fragment>
			Avoid casting <ActionLink {...ACTIONS.PAINFLARE
		} /
 		>
		on;
	a;
	...
	Fragment > ,
		severity:;
	numBadPainflares < 5 ? SEVERITY.MEDIUM : SEVERITY.MAJOR,
		why:; <
	Fragment >
		{ numBadPainflares };
	single - target;
