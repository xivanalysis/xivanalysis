...
export default class Ruin2 extends Module {
	...
	_onComplete() {
		...
		if (issues) {
			...
			Fragment > ,
				why:;
			(issues * potLossPerR2) + " potency lost to unnecessary Ruin II casts.",
				severity:;
			issues < 5 ? SEVERITY.MINOR : issues < 10 ? SEVERITY.MEDIUM : SEVERITY.MAJOR, ;
		}))
}
...
if (warnings) {
	this.suggestions.add(new Suggestion({
			...
			content: <Fragment>
			Unless significant movement is required,
			avoid using <ActionLink {...ACTIONS.RUIN_II
		} /
 		>
	;
	for movement.Most position adjustments can;
	be;
	performed;
	with slidecasting and;
	the;
	additional;
	mobility;
	available;
	...
	Fragment > ,
		why:;
	(warnings * potLossPerR2) + " potency lost to Ruin II casts used only to move.",
		severity:;
	warnings < 5 ? SEVERITY.MINOR : warnings < 10 ? SEVERITY.MEDIUM : SEVERITY.MAJOR, ;
}))
}
}
}
