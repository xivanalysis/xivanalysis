	...
	_onComplete() {
		...
		if (badGcds) {
			this.suggestions.add(new Suggestion({
					...
					content: <Fragment>
					GCDs used during Dreadwyrm Trance should be limited to <ActionLink {...ACTIONS.RUIN_III
				} /
 				>
				and <
				ActionLink;
...
if (this._missedGcds) {
	...
	this.suggestions.add(new Suggestion({
				icon: ACTIONS.DREADWYRM_TRANCE.icon,
				content: <Fragment>
				You can fit <strong>{possibleGcds
			} <
 		/
		strong >
		GCDs in
	...
	Fragment > ,
		severity:;
	this._missedGcds < 10 ? SEVERITY.MINOR : SEVERITY.MEDIUM,
		why:; `${
...
if (this._missedDeathflares) {
	this.suggestions.add(new Suggestion({
			...
			content: <Fragment>
			Make sure you always end <ActionLink {...ACTIONS.DREADWYRM_TRANCE
		} /
 		>
	;
	with a < ActionLink {
