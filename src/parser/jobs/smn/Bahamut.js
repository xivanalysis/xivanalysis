...
export default class Bahamut extends Module {
	...
	_onBahamutCast(event) {
		...
		const ghostChance = timeSinceSummon >= SUMMON_BAHAMUT_LENGTH
			? GHOST_CHANCE.ABSOLUTE
			: timeSinceSummon < SUMMON_BAHAMUT_LENGTH - GHOST_TIMEFRAME
			? GHOST_CHANCE.NONE
			: GHOST_CHANCE.LIKELY;
	...
	output() {
		...
		- & nbsp;
		{
			this.renderHeaderCount(counts[ACTIONS.WYRMWAVE.id])
		}
		WWs, &;
		nbsp;
		{
			this.renderHeaderCount(counts[ACTIONS.AKH_MORN.id])
		}
		AMs < /;
		Fragment > , ;
	},
	...
	className={ GHOST_CLASSNAME[cast.ghostChance] } >  < strong >
		{ this.parser.formatDuration(cast.timestamp - sb.timestamp, 2)
}:
<;/
strong >  & nbsp;
{
...
renderHeaderCount(counts);
{
	...
	return [
		...
		GHOST_CHANCE.LIKELY,
		GHOST_CHANCE.ABSOLUTE,
	].map((chance, i) => counts[chance] &&  < Fragment;
	key = { chance } >
		{ i > 0 && '/' } <
		span;
	className = { GHOST_CLASSNAME[chance] } > { counts[chance] } < /;
	span >  < /;
	Fragment > );
}
}
