...
curl -v -H User-Agent:bot -H Content-Type:application/json -d "{\"embeds\":[{\"author\":{\"name\":\"Build #$GITHUB_RUN_NUMBER ($GITHUB_REF_NAME) Passed\",\"url\":\"$WORKFLOW_URL\"},\"url\":\"$COMPARE_URL\",\"color\":4303448,\"description\":\"$COMMIT_FORMATTED - $COMMIT_MESSAGE\"}]}" $DISCORD_WEBHOOK_URL;
