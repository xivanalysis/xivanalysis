...
curl -v -H User-Agent:bot -H Content-Type:application/json -d "{\"embeds\":[{\"author\":{\"name\":\"Build #$((CIRCLE_BUILD_NUM - 1)) Failed\",\"url\":\"$CIRCLE_BUILD_URL\"},\"url\":\"$CIRCLE_COMPARE_URL\",\"color\":13700128,\"description\":\"$COMMIT_FORMATTED - $COMMIT_MESSAGE\"}]}" $DISCORD_WEBHOOK_URL;
